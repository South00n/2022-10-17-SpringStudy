...
public class SeoulController {
	...
	@GetMapping("seoul/location.do") // if => 어노테이션 (index : 찾기) => 사용자 요청에 따라 메소드를 찾는다
	...
	@GetMapping("seoul/nature.do") // if => 어노테이션 (index : 찾기) => 사용자 요청에 따라 메소드를 찾는다
	public String seoul_nature(String page, Model model) {
		...
		map.put("start", (curpage*20)-19);
		map.put("end", curpage*20);
		List<SeoulNatureVO> sList = dao.seoulNatureListData(map);
		int totalpage = dao.seoulNautreTotalPage();
	...
	@GetMapping("seoul/shop.do") // if => 어노테이션 (index : 찾기) => 사용자 요청에 따라 메소드를 찾는다
	public String seoul_shop(String page, Model model) {
		...
		map.put("start", (curpage*20)-19);
		map.put("end", curpage*20);
		List<SeoulShopVO> sList = dao.seoulShopListData(map);
