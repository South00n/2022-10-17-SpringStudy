...
/*
 * 
 * 	Spring4 = XML, Annotation => 정보가 노출 (보안 취약)
 *  SPring5 = 자바로 환경설정 (.class) => 보안에 주력 => 차세대 ==> Spring-Boot
 *  
 *  = 모든 클래스를 등록하는 것이 아니다 (기능(컴포넌트)를 가지고있는 클래스 => 여러 클래스에서 호출)
 *    1) 등록에서 제외 : 데이터형 (자바 기본데이터형, 사용자 정의 데이터형 : ~VO)
 *    2) 생성/ 소멸부분은 스프링이 담당
 *    3) 특별한 경우가 아니면 싱글턴 (한개의 메모리주소를 이용해서 사용)
 *    				  ----- 기술면접 1순위
 *    				  ----- DI/AOP
 *     				  ----- MVC
 *     				  ----- Transaction
...
public class MainClass {
	 ...
	 *       = 객체 소멸  => destory-method "메소드명만"
	 *     =========================================================  
	 *     XML에 등록
	 *     1) 사용자 정의 클래스
	 *        = 패키지단위로 등록 => Annotation
